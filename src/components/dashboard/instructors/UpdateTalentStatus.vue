...
const updateTalentStatus:any = async () => {
    console.log('I will update talent status');
    console.log('talent id should be', talent.value.id)
    console.log('status value should be', statusId.value)
    // route.push('/dashboard/student-management')
    const data:any = {
        talentId: talent.value,
    }
    const request:any = `${api_url}api/talentpool/updatestatus/${talent.value.id}/${statusId.value}`;
    ...
    await store.dispatch(actionTypes.UpdateTalentStatus, request)
    closeModal()
}
    ...
    <div class="main w-full mt-[0.5px] px-[45px] h-screen bg-white">
        ...
        <form class="text-sm grid">
            ...
            <div class="flex justify-end pb-10">
                <button @click.prevent="updateTalentStatus" class="py-4 px-8 hover:bg-opacity-80 font-bold flex justify-center border bg-primary text-white rounded-md">Save Changes</button>
            </div>
        </form>
    </div>
</template>
