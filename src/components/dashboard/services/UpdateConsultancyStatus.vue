...
const consultancy:any = computed(() => {
  ...
  return JSON.parse(JSON.stringify(store.getters.getEditConsultancy.value))
})
...
const updateConsultancyStatus:any = async () => {
    console.log('I will update consultancy status');
    console.log('consultancy id should be', consultancy.value.id)
    console.log('status value should be', statusId.value)
    // route.push('/dashboard/student-management')
    // const data:any = {
    //     consultancyId: consultancy.value,
    // }
    const request:any = `${api_url}api/consultancy/updatestatus/${consultancy.value.id}/${statusId.value}`;
    ...
    await store.dispatch(actionTypes.UpdateConsultancyStatus, request)
    closeModal()
    setTimeout(() => {
        statusId.value = ''
    }, 1000)
...
onMounted(async() => {
    ...
    const request:any = `${api_url}api/consultancy/get-consultancys/{pageIndex}/{pageSize}`;
...
<template>
    <div class="main relative w-full px-6 lg:px-[45px] h-screen bg-white">
        <div class="flex justify-between py-[53px] items-center ">
            <p class="text-2xl">Update Consultancy Status</p>
        ...
        <form class="text-sm text-left grid">
            ...
            <div class="flex justify-end pb-10">
                <button @click.prevent="updateConsultancyStatus" class="py-4 px-8 hover:bg-opacity-80 font-bold flex justify-center border bg-primary text-white rounded-md">Save Changes</button>
            </div>
        </form>
    </div>
</template>
