...
import IPODetails from './IPOQuoteDetails.vue';
...
const contacts:any = computed(() => {
    return store.getters.getIPOQuotes.value.payload;
})

const total_count:any = computed(() => {
    return store.getters.getIPOQuotes.value.totalCount;
})
...
const onPageChange:any = async (page:any) => {
    ...
    const request:any = `${api_url}api/iposquoterequest/get-iposquote/${pageIndex.value}/{pageSize}`;
...
const setId:any = (id:any) => {
    ...
    store.dispatch(actionTypes.FetchEditIPOQuote, request)
}
...
<template>
    <div class="main grid">
        <div class="title flex justify-between items-center mb-10">
            <h1 class="text-2xl font-semibold text-black">Quote Request List</h1>
                      ...
                      <td class="border-t-0 px-3 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                        <div class="flex w-2/5 items-center">
                            ...
                            <DeleteModal :show="showDelete" @close="showDelete = !showDelete" @delete="deleteIPO(contactitemtodelete)">
                                    ...
                                    <template #info>
                                        Are you sure you want to remove quote request?
                                    </template>
                                    <template #delete>
                                        Yes, Delete IPO
                                    </template>
